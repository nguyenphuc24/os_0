...
## 2. Cấu Trúc Dự Án Hiện Tại và Cách Chạy

### 2.1 Cấu trúc thư mục
- `src/`: Chứa mã nguồn C cốt lõi của dự án (CPU, Memory, Scheduler, Loader).
- `include/`: Chứa các file header (`.h`).
- `input/`: Chứa các kịch bản test (process test cases, memory test, sched test như `os_1_mlq_paging`, v.v.).
- `test/`: Chứa kết quả output (dạng `.output`) sau khi chạy các test cases.
- `Makefile` & `run.sh`: Các script hỗ trợ biên dịch và chạy test hàng loạt.

### 2.2 Cách Biên Dịch và Chạy Test
Sử dụng terminal (Bash, MinGW, hoặc WSL):
...
**2. Chạy hàng loạt các test cases:**
```bash
./run.sh
```
Kết quả output sẽ được tự động lưu vào thư mục `test/`.

**3. Chạy đơn lẻ một test case:**
```bash
./os <tên_file_trong_thư_mục_input>
# Ví dụ:
./os os_1_mlq_paging
#chạy sched thì thêm cấu hình memory như hàng thứ 2 của os_syscall, hoặc bật MM_FIXED_MEMSZ trong os-cfg
```
...
> **Ghi chú thiết kế**: Hệ thống sử dụng trường `unsigned long mode_bit` trong `vm_rg_struct` để phân biệt usermode (mode_bit=1) và kernelmode (mode_bit=0). Code hiện tại đã implement đầy đủ việc kiểm tra và gán giá trị này trong các hàm `__alloc`, `__kmalloc` và các hàm đọc/ghi để đảm bảo tính cô lập bộ nhớ.
...
`KERNEL_BASE = 0xfe00000000000000ULL`
